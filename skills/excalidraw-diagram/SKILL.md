...
### Step 6: Render & Validate (Optional)
Skip this step by default. Only render and validate if the user explicitly asks, points out a visual mistake, or requests a fix. See the **Render & Validate** section below for the process when needed.
...
**Phase 3: Render & validate (optional)**

Only run the render-view-fix loop if the user explicitly requests it or reports a visual issue.
...
## Render & Validate (On Request Only)

Only render when the user explicitly asks, or when they point out a visual mistake. Do not render by default after generating JSON.
...
### Visual Validation (On Request Only)
Only run these checks if the user asks for validation or reports a visual issue:
